...
if (isset($_POST['gallery-submit'])) {
    ...
    if(!in_array($ext, $allowed)){
        header("Location:../admin.php?error=UnknownFileType");
        exit();
    }
    if($file_size > 4 * MB){
        ...
        $sql = "INSERT INTO gallery (title, descript, picpath) VALUES (?, ?, ?)";//picpath='$destination' WHERE uname='$uname'
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            ...
            header("Location:../admin.php?success=UploadSuccessful");
            exit();
        }
